...
USE_GL="${USE_GL:-0}"
if [ "$USE_GL" = "1" ]; then
    # 修改：添加 max_outputs=2 参数
    GPU_DEVICE="virtio-gpu-gl-pci,iommu_platform=on,disable-legacy=on,edid=on,max_outputs=2,xres=1024,yres=768"
    DISPLAY_ARGS=( -display gtk,show-cursor=on,gl=on )
else
    # 修改：添加 max_outputs=2 参数
    GPU_DEVICE="virtio-gpu-pci,iommu_platform=on,disable-legacy=on,edid=on,max_outputs=2,xres=1024,yres=768"
